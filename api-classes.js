...
class StoryList {
  ...
  // @ This Function Asychronizes Stories from BASE URL API Website
  // @ Method to generate new StoryList
  static async getStories() {

    // query the /stories endpoint (no auth required)

    // @ Awaits reponse to retrieves stories from BASE URL
    // @ builds array of story instances
    const response = await axios.get(`${BASE_URL}/stories`);

    // turn plain old story objects from API into instances of Story class

    // @ ???????????? Allows user to input new Story
    // @ makes a single Storylist instance out of that
    const stories = response.data.stories.map(story => new Story(story));

    // build an instance of our own class using the new array of stories

    //  @ ?????????? Appends Storylist on browser
    // @ returns the Storlist instance
